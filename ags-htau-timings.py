...
@jit(nopython=True)
def simulate_ags_htau_final_time(
    ...
    while t < T:
        ...
        X, t = cs.hybrid1_tau_step_numba(X, t, V, props, betas, TSS[0], TSS[1])

    return X


def simulate_ags_htau(T: float, V: np.array, css: np.array, IS: tuple, TSS: tuple):
    props = np.zeros((6,))
    betas = np.zeros((6,))
    X = np.array([1.0, 0.0, 0.0, 0.0])
    t = 0.0
...
def time_paths(
    ...
    simulate_ags_htau_final_time(T, V, css, IS, TSS)
...
def xp1():
    ...
    for i, IS in enumerate(ISs):
        for j, TSS in enumerate(TSSs):
            ...
            np.save(f"dat/ags/ags-htau-times-IS{i}-TS{j}", times)
...
def xp2():
    ...
    for i, IS in enumerate(ISs):
        for j, TSS in enumerate(TSSs):
            ...
            X_hist, t_hist = simulate_ags_htau(T, V, css, IS, TSS)
    ...
    for i, IS in enumerate(ISs):
        for j, TSS in enumerate(TSSs):
            ...
            for k in tqdm.tqdm(range(Npaths)):
                dat[k] = simulate_ags_htau_final_time(T, V, css, IS, TSS)

            np.save(f"dat/ags/ags-htau-states-IS{i}-TS{j}", dat)
