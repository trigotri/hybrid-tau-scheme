...
def time_lv():
    ...
    for i in tqdm.tqdm(range(N)):
        time_dat[i] = record_times_hcle(T, V, ks, save_at_ts,I1, I2, I1, I2, delta_t, Delta_t)

    np.save(f'dat/hcle-times', time_dat)
    np.save(f'dat/hcle-save-times', save_at_ts)
...
def save_MC_dat(dat : np.array):
    np.save(f'dat/hcle-{dat.shape[0]}-{dat.shape[1]}', dat)
